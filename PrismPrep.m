function []=PrismPrep(Array)

N_grain=length(Array);
%%
%% Part 2
for i=1:length(Array)
    %Input Euler angles pertaining to each grain in .log file: Columns 3:5
    %Ang(i,:)=[i, 0, log_data(i).euler];
    Ang(i,:)=[i, 0, Array(i,10),Array(i,11),Array(i,12)];% Note Ang(1,:) corresponds to grain 1 and so on;
end


%  combine the two percent symbols to split this cell to establish 
%    the Ang variable space, into which more data can be pasted 

%Read the stress tensors from output of
%procGVE_multi_stress_correct_N13 [Sample Coordinate System]
...
% Stress Tensors in Grain Coordinate System
%sigmagrain=zeros(3,3,N_grain); % Initialize array to store stress tensors in grain coordinate system
%for j=1:length(log_data)
 %   sigmagrain(:,:,j)=log_data(j).U*sigma(:,:,j)*log_data(j).U'; %Sigmaprime=g*Sigma*gTranspose
%end
